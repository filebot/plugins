...
# generated by [picocli](http://picocli.info/) version 4.1.2.
#
# Installation
# ------------
#
...
function _picocli_filebot() {
  ...
  COMMANDS=""
  FLAG_OPTS="-rename -non-strict -get-subtitles -check -list -mediainfo -revert -extract -r -unixfs -no-xattr -no-probe -no-history -clear-cache -clear-prefs -clear-history -version -help"
  ARG_OPTS="--mode --db --order --action --conflict --filter --mapper --format --q --lang --output --encoding -script --def --file-filter --apply -exec --log --log-file --license"
  ...
  apply_OPTION_ARGS="artwork cover nfo url metadata import subtitles date tags prune clean refresh" # --apply values
  ...
  case ${PREV_WORD} in
    ...
    --file-filter)
      ...
      ;;
    --apply)
      COMPREPLY=( $( compgen -W "${apply_OPTION_ARGS}" -- ${CURR_WORD} ) )
      return $?
      ;;
    -exec)
      return
